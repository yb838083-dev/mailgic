...
function Navbar({onNav,user,onLogout,page}) {
  ...
  return (
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:200,padding:"0 24px",background:scrolled||!isHome?"rgba(255,255,255,.96)":"transparent",backdropFilter:scrolled||!isHome?"blur(18px)":"none",borderBottom:scrolled||!isHome?"1px solid rgba(124,58,237,.1)":"none",transition:"all .3s ease"}}>
      <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:68}}>
        <div onClick={()=>onNav("home")}><Logo size={28}/></div>
        {isHome&&(
          <div style={{display:"flex",gap:26}}>
            {[["Features","features"],["Generator","generator"],["Pricing","pricing"],["Testimonials","testimonials"]].map(([l,id])=>(
              <button key={id} className="nl" onClick={()=>onNav(id)}>{l}</button>
            ))}
          </div>
        )}
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          {user?(
            <>
              <button className="nl" style={{color:P,fontWeight:600,fontSize:13,display:"flex",alignItems:"center",gap:5}} onClick={()=>onNav("dashboard")}>
                <User size={14}/> Dashboard
              </button>
              <button className="obtn" style={{padding:"8px 16px",fontSize:13,gap:6}} onClick={onLogout}>
                <LogOut size={14}/> Sign out
              </button>
            </>
          ):(
            <>
              <button className="nl" style={{padding:"9px 20px",borderRadius:12,fontSize:13}} onClick={()=>onNav("login")}>Log in</button>
              <button className="gbtn" style={{padding:"9px 20px",fontSize:13}} onClick={()=>onNav("signup")}>
  ...
  return (
    ...
    <section id="home" style={{minHeight:"100vh",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",background:"#1a0533",overflow:"hidden",padding:"100px 24px 60px",isolation:"isolate"}}>
      ...
      <div style={{position:"relative",zIndex:1,textAlign:"center",maxWidth:820,margin:"0 auto"}}>
        ...
        <div className="su4" style={{marginTop:52,display:"flex",justifyContent:"center",gap:44,flexWrap:"wrap"}}>
        ...
        <div className="su4 flt card" style={{maxWidth:400,margin:"44px auto 0",padding:"20px 24px",textAlign:"left",background:"rgba(255,255,255,.97)",boxShadow:"0 20px 60px rgba(0,0,0,.3)",border:"1px solid rgba(124,58,237,.25)"}}>
  ...
  return (
    <section id="generator" style={{padding:"100px 24px",background:"#fff",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:860,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        <div className="card" style={{padding:"40px",boxShadow:"0 24px 80px rgba(124,58,237,.1)"}}>
  ...
  return (
    <div style={{overflow:"hidden",height:480,position:"relative"}}>
      <div className={rev?"cbwd":"cfwd"} style={{display:"flex",flexDirection:"column",gap:14}}>
        {d.map((t,i)=>(
          <div key={i} className="card" style={{padding:"18px 20px",margin:"0 4px"}}>
            <p style={{fontSize:13,color:"#374151",lineHeight:1.72,marginBottom:14,fontFamily:"Plus Jakarta Sans"}}>"{t.tx}"</p>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <img src={t.av} alt={t.nm} style={{width:34,height:34,borderRadius:"50%",border:"2px solid rgba(124,58,237,.2)"}} loading="lazy"/>
              <div>
    ...
    <section id="pricing" style={{padding:"100px 24px",background:"#fafbff",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:1000,margin:"0 auto",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,alignItems:"start"}}>
          {PLANS.map((plan,i)=>(
            <div key={plan.nm} className={`card sp${i} ${plan.pop?"pp":""}`} style={{padding:"32px 28px",position:"relative",marginTop:plan.pop?0:16}}>
    ...
    <footer style={{background:"#0f0a1e",padding:"64px 24px 32px"}}>
      <div style={{maxWidth:1100,margin:"0 auto"}}>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,.08)",paddingTop:28,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}>
          <p style={{fontSize:13,color:"#6b7280",fontFamily:"Plus Jakarta Sans"}}>© 2025 Mailgic. All rights reserved.</p>
    ...
    try{
      if(isLogin){
        ...
        if(d.access_token&&d.user?.email_confirmed_at){
          ...
          const usr={id:d.user.id,email:d.user.email,token:d.access_token,name:d.user.user_metadata?.full_name||name};
          onAuth(usr); onNav("dashboard");
        }else if(d.user){
          // Email confirmation required OR session available
          if(d.session?.access_token){
            const usr={id:d.user.id,email:d.user.email,token:d.session.access_token,name:d.user.user_metadata?.full_name||name};
            onAuth(usr); onNav("dashboard");
          }else{
            setMsg("Account created! Check your email to confirm your account, then log in here.");
          }
        }else{
      ...
      const m=e.message||"";
      if(m.includes("Failed to fetch")||m.includes("NetworkError")||m.includes("fetch")) setErr("Network error — please check your connection and try again.");
      else if(m.includes("Email not confirmed")) setErr("Please confirm your email first, then try logging in.");
      else if(m.includes("Invalid login")) setErr("Invalid email or password. Please try again.");
      else if(m.includes("already registered")) setErr("This email is already registered. Try logging in instead.");
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                ...
                {isLogin&&<button className="nl" style={{fontSize:12,color:P}}>Forgot password?</button>}
              </div>
              <div style={{position:"relative"}}>
...
function Dashboard({user,onNav,onLogout}) {
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8f6ff",paddingTop:68}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1a0533 0%,#4c1d95 100%)",padding:"48px 24px 56px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:1100,margin:"0 auto",position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
            ...
            <button className="gbtn" style={{padding:"12px 24px",fontSize:14}} onClick={()=>onNav("generator")}>
      ...
      {/* Content */}
      <div style={{maxWidth:1100,margin:"0 auto",padding:"36px 24px 80px",display:"grid",gridTemplateColumns:"1fr 1.4fr",gap:24,alignItems:"start"}}>
        {/* Email history list */}
        <div>
          ...
          {loading?(
            ...
            <div className="card" style={{padding:"40px",textAlign:"center"}}>
              ...
              <button className="gbtn" style={{padding:"10px 20px",fontSize:13,marginTop:16}} onClick={()=>onNav("generator")}>Generate your first email</button>
            </div>
          ):(
                ...
                <div key={g.id||i} onClick={()=>setSelected(selected?.id===g.id?null:g)}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontFamily:"Plus Jakarta Sans",fontWeight:600,fontSize:13.5,color:"#111827",marginBottom:4,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{g.topic}</p>
